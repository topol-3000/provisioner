...
- [x] 01-02-PLAN.md — Worker source + test suite: settings.py, infrastructure/*.py (real boot path), __main__.py, main.py (TaskGroup + SIGTERM), placeholder module tree, all unit tests

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 01-03-PLAN.md — Container + CI: Dockerfile (two-stage, uv, non-root), docker-compose.yml (external platform-net), .github/workflows/ci.yml (lint + test + build-only)
...
- [x] 02-02-PLAN.md — ProcessedEvent ORM + Alembic migration + settings: models.py, migration add_processed_event, env.py update, settings.consumer_reclaim_min_idle_ms
...
- [x] 03-03-PLAN.md — Convergence engine: service.py + tasks.py (create_instance_task with backoff) + handlers.py real body + main.py wiring — end-to-end slice delivers subscription.activated → ready
